...
struct MenuBarView: View {
    ...
    // Read lazily: LaunchAtLogin.isEnabled is a blocking SMAppService XPC call,
    // and this initializer runs on every menu-bar view rebuild (upstream #848).
        ...
        Group {
            ...
            Toggle("Launch at Login", isOn: $launchAtLoginEnabled)
                .onChange(of: launchAtLoginEnabled) { oldValue, newValue in
                    guard hasLoadedLaunchAtLogin, LaunchAtLogin.isEnabled != newValue else { return }
                    LaunchAtLogin.isEnabled = newValue
                }
                .task {
                    guard !hasLoadedLaunchAtLogin else { return }
                    let enabled = await Task.detached { LaunchAtLogin.isEnabled }.value
