...
struct DashboardHeroCard: View {
    private static let headlineFont: Font = .system(size: 22, weight: .bold)
    private static let highlightedHeadlineFont: Font = .system(size: 26, weight: .bold)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            // The landing page shows insights inline now, so the locked card
            // carries no navigation button: the stats below tell the story.
            if !isLocked {
                HStack(spacing: 12) {
    ...
    private var lockedInsightsPrompt: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ProgressView(value: min(max(unlockProgress, 0), 1))
                ...
                .tint(DashboardMomentumBackground.accent)
                .frame(maxWidth: 420)
