...
struct DashboardContent: View {
    ...
    private static let displayNameFontSize: CGFloat = 26
    ...
    var body: some View {
        ...
        .task(id: isAccessibilityEnabled) {
            // AXIsProcessTrusted can stay stale after the user grants access in
            // System Settings; poll while the banner is up so it clears without
            // an app restart.
    ...
    private var landingInsights: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            DashboardProductivityCard(
                period: $selectedInsightPeriod,
                points: selectedProductivityPoints,
            ...
            HStack(alignment: .top, spacing: DashboardLayout.columnSpacing) {
                ...
                DashboardTimeSavedCard(summary: selectedTimeSavedSummary, period: selectedInsightPeriod)
                    .frame(maxWidth: .infinity, alignment: .topLeading)
            }
            .frame(height: 196)
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                ...
                InsightPeriodPicker(
                    ...
                    selection: $selectedInsightPeriod
                )
            }
    ...
    private var displayNameFont: Font {
        .system(size: Self.displayNameFontSize, weight: .semibold, design: .serif).italic()
    }
    ...
    private var insightsUnlockProgress: Double {
        guard Self.insightsUnlockDuration > 0 else { return 1 }
        return min(statsSummary.totalDuration / Self.insightsUnlockDuration, 1)
    }

    private var insightsUnlockMinutesRemaining: Int {
        let remaining = Self.insightsUnlockDuration - statsSummary.totalDuration
        guard remaining > 0 else { return 0 }
        return max(Int((remaining / 60).rounded(.up)), 1)
    }
